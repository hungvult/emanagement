...
import com.emanagement.backend.modules.face.FaceCandidateCacheService;
...
public class KioskServiceImpl implements KioskService {
    ...
    private final FaceCandidateCacheService faceCandidateCacheService;
    ...
    public KioskCheckInResponseDto processCheckIn(String deviceToken, KioskCheckInRequestDto requestDto) {
        ...
        // 1. Tải danh sách tất cả các nhân viên đã đăng ký khuôn mặt từ In-Memory Cache
        // (0ms latency)
        List<AiCandidateDto> candidates = faceCandidateCacheService.getCandidates();
        ...
        User user = userRepository.findById(matchedUserId)
                .orElseThrow(
                        () -> new ResourceNotFoundException("Không tìm thấy thông tin nhân viên ID: " + matchedUserId));
        ...
        if (todayRecords.isEmpty()) {
            ...
            if (record.getCheckOutTime() != null) {
                throw new BusinessException(
                        "Bạn đã hoàn thành việc chấm công hôm nay (vào & ra ca). Không thể chấm công thêm.");
            }

            // Tránh trường hợp vừa check-in xong đứng nán lại bị máy tự quét nhầm thành
            // check-out
            if (record.getCheckInTime() != null
                    && java.time.Duration.between(record.getCheckInTime(), now).toMinutes() < 1) {
                throw new BusinessException(
                        "Bạn vừa mới chấm công vào ca thành công! Vui lòng quay lại sau để chấm công ra ca.");
            }
        ...
        return KioskCheckInResponseDto.builder()
                ...
                .message((checkType.equals("CHECK_IN") ? "Check-in" : "Check-out") + " thành công cho nhân viên "
                        + user.getFullName())
                .build();
    }
            ...
            if (jwtTokenProvider.validateToken(deviceToken)) {
                return kioskRepository.findByDeviceToken(deviceToken)
                        .orElseThrow(() -> new ResourceNotFoundException(
                                "Xác thực Kiosk thất bại: Trạm Kiosk không tồn tại trên hệ thống"));
            }
        }
